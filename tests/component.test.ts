...
import { createComponentProxy } from "../src";
...
describe("component", () => {
  ...
  it("Debe emitir eventos de componentes", () => {
    ...
    const instance = createComponentInstance(Child, {
      onChange: (value: number) => {
        received = value;
      },
    });

    instance.emit("change", 42);
  ...
  it("Deberia de exponer el estado de un setup a través de un proxy de un componente", () => {
    ...
    expect(instance.proxy.count).toBe(10);
  });
  it("Deberia de exponerse los props a través del proxy del componente", () => {
    ...
    expect(instance.proxy.title).toBe("Hello");
  });
  it("Deberia priorizar el estado del setup por encima de los props", () => {
    ...
    expect(instance.proxy.title).toBe("setup");
  });
  ...
  it("Deberia actualizar ref a través del proxy del componente", () => {
    ...
    expect((instance.setupState.count as Ref<number>).value).toBe(20);

    expect(instance.proxy.count).toBe(20);
  });
  ...
  it("Deberia registrar metadata el componente", () => {
    ...
    const metadata = getComponentMetadata(UserCard);

    expect(metadata).toBeDefined();
    expect(metadata?.selector).toBe("user-card");

    expect(metadata?.type).toBe(UserCard);
  });
  it("expone el estado de setup a través de la instancia pública", () => {
    @Component()
    class Counter {
      setup() {
        return {
    ...
    const proxy = createComponentProxy(instance);

    expect(proxy.count.value).toBe(10);
  });
  it("expone props a través de la instancia pública", () => {
    @Component()
    class Counter {}
    ...
    expect(proxy.initial).toBe(42);
  });
  it("prioriza setupState sobre props", () => {
    @Component()
    class Counter {
      setup() {
        return {
